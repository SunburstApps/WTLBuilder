...
LRESULT CMainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	ResizeClient(774, 124);
	SetWindowText(_T("WTL Builder"));
...
LRESULT CMainFrame::OnFileRecent(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	else
	{
		...
		::MessageBox(m_hWnd, _T("Cannot get file name from mru list"), _T("WTL Builder"), MB_OK | MB_ICONERROR);
	}
	return 0;
}
...
void CMainFrame::SetActiveForm(Component *form)
{
	...
	if (form != NULL)
	{
		...
		SetWindowText(MakeString(_T("WTL Builder - [%s]%c"), (LPCTSTR)formFileName, flag ? '*' : ' '));
	}
	else
	{
		SetWindowText(_T("WTL Builder"));
